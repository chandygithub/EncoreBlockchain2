...
function checkCard()
{
	...
	if (options.registry == 'Bank'){

	$.when($.post('/composer/admin/getMembers', options),$.get('Bank.html')).done(function (_results, _page)
    {
...
function issueIdentity()
{
	 ...
	 $.when($.post('/composer/admin/getMembers', options),$.get('Bank.html')).done(function (_results, _page)
    {
        $('#admin-forms').append(_page[0]);
        $('#member_type').append(options.registry);
	console.log("options.registry" + options.registry);
        updatePage('removeMember');
        member_list = _results[0].members;
	console.log("_results[0].members" + _results[0].members);
        for (let each in _results[0].members)
        {(function(_idx, _arr){
...
{
    let options = {};
    options.registry = 'Program';
    options.type='admin';
    $.when($.get('/composer/admin/getProgram/:programId', options)).done(function (_results)
    {
        let _str = '';
        _str +='<h2>Registry List</h2>';
        _str += '<h4>Network update results: '+_results.result+'</h4>';
        ...
        {
            _str += '<table width="100%"><tr><th>Name</th><th>Type</th><th>Limit</th><th>Rate of Interest</th><th>Program Balance</th></tr>';
            for (let each in _results.programs)
            {(function(_idx, _arr){
                _str += '<tr><td align="center">'+_arr[_idx].programName+'</td><td>'+_arr[_idx].programType+'</td><td>'+_arr[_idx].programLimit+'</td><td>'+_arr[_idx].programROI+'</td><td>'+_arr[_idx].programDiscount+'</td></tr>';
            })(each, _results.programs);}
            _str += '</ul>';
...
{
let _str;
console.log("inside view Transaction");
    $.when($.get('viewTransaction.html')).done(function (_page)
    {
        $('#admin-forms').empty();
        $('#admin-forms').append(_page);
        updatePage('viewTransaction');
        let _cancel = $('#cancel');
        let _submit = $('#submit');
        $('#messages').empty();
        $('#messages').append('<br/>Please fill Program ID.');
        _cancel.on('click', function (){$('#admin-forms').empty();});
        _submit.on('click', function(){
            $('#messages').append('<br/>Searching Transaction.');
                 let options = {};

			  	 options.programID = $('#programID').val();
			 $.when($.get('/composer/admin/viewtransaction/:programId*', options)).done(function(_results)
            	{
					let data =_results.data;
					console.log("_results.result" +_results.result)
					console.log("_results.data" +_results.data)
				if (_results.result === 'success')
				{
				 _str = '';
				    _str += '<table width="100%"><tr><th>Program ID</th><th>Program Name</th><th>PPR</th><th>Transaction Id</th><th>Timestamp</th></tr>';
				console.log("Length of response data " +data.length);
				for(let i=0; i < data.length ;i++){
					...
					data[i].timestamp;
				   _str += '<tr><td>'+data[i].program+'</td><td>'+data[i].programName+'</td><td>'+data[i].pprobjref+'</td><td>'+data[i].transactionID+'</td><td>'+data[i].timestamp+'</td></tr>';
console.log("program " + data[i].program + "programName " +data[i].programName )
			}
				_str +='</table>'
				} else {_str += '<br/>'+_results.error;} 
  		 $('#admin-forms').empty();
         $('#admin-forms').append(_str);
		  		//console.log("Result :"+ _res.result);
				//console.log("Result :"+ _res.data);
				// $('#messages').append(formatMessage(_res.data)); 
		});
	});
   });
}
...
$.when($.get('addTransaction.html')).done(function (_page)
    {
        $('#admin-forms').empty();
        $('#admin-forms').append(_page);
		//Update the html page
        updatePage('addTransaction');
        let _cancel = $('#cancel');
        let _submit = $('#submit');
        $('#messages').empty();
        $('#messages').append('<br/>Please fill in Transaction  form.');
        _cancel.on('click', function (){$('#admin-forms').empty();});
        _submit.on('click', function(){
            $('#messages').append('<br/>Starting Transaction Request.');
            let options = {};
                      
			options.pprID = $('#pprID').val();
			options.programID = $('#ProgramID').val();
			options.programBussinessROI = $('#programBussinessROI').val();
			options.programBussinessLimit = $('#programBussinessLimit').val();
			options.programBussinessDiscount = $('#programBussinessDiscount').val();
			options.programBussinessDiscountPercent = $('#programBussinessDiscountPercent').val();
			console.log("options " +options)
			//post call , where the links are saved in router.js 
            $.when($.post('/composer/admin/transaction', options)).done(function(_res)
            { $('#messages').append(formatMessage(_res)); });
        });
    });
}
/**
 this method was called when the addmember option is clicked in admin.html
 * add a member to a registry
 */
function addMember1(){
	let options = $('#registryName1').find(':selected').text();
	alert("options" +options);
	// gets the dropdown registry value from UI , opens the html page accordingly
	console.log("options " +options);
	if(options == "Bank"){
		addBank();
	} 
...
function sampleData(){
 ...
 console.log("inside sample RestAPI");
 $.when($.get('sampleData.html')).done(function (_page)
    {
        $('#admin-forms').empty();
        $('#admin-forms').append(_page);
        updatePage('sampleData');
        let _cancel = $('#cancel');
        let _submit = $('#submit');
        $('#messages').empty();
        $('#messages').append('<br/>Please fill in sample form.');
        _cancel.on('click', function (){$('#admin-forms').empty();});
        _submit.on('click', function(){
            $('#messages').append('<br/>starting sample request.');
            let options = {};
			options.accountNo = $('#accountNo').val();
			options.Name = $('#Name').val();
			options.IFSC = $('#ifsc').val();
		$.when($.post('/composer/admin/addsampleData', options)).done(function(_res)
				    { $('#messages').append(formatMessage(_res)); });
			});
		});
}
...
{
console.log("inside add bank member");
    $.when($.get('Business.html')).done(function (_page)
    {
        $('#admin-forms').empty();
        $('#admin-forms').append(_page);
        updatePage('Business');
        ...
        _submit.on('click', function(){
			...
			options.businessAccNo = $('#businessAccNo').val();
			options.businessLimit = $('#businessLimit').val();
			options.businessWalletId = $('#businessWalletId').val();
			options.businessLoanWalletId = $('#businessLoanWalletId').val();
			options.minROI = $('#minROI').val();
			options.maxROI = $('#maxROI').val();
			options.businessLiablityWallet = $('#businessLiablityWallet').val();
			options.p1RepaymentAcNo = $('#p1RepaymentAcNo').val();
			options.pnRepaymentAcNo = $('#pnRepaymentAcNo').val();

            $.when($.post('/composer/admin/addBusinessMember', options)).done(function(_res)
            { $('#messages').append(formatMessage(_res)); });
        });
    });
}
...
{
console.log("inside add bank member");
    $.when($.get('bankMember.html')).done(function (_page)
    {
        $('#admin-forms').empty();
        $('#admin-forms').append(_page);
        updatePage('bankMember');
        let _cancel = $('#cancel');
        let _submit = $('#submit');
        $('#messages').empty();
        $('#messages').append('<br/>Please fill in add member form.');
        ...
        _submit.on('click', function(){
			...
			options.bankWalletID = $('#bankWalletID').val();
			options.bankAssetWalletId = $('#bankAssetWalletId').val();
			options.bankLiabilityWalletId = $('#bankLiabilityWalletId').val();
			options.bankChargesWalletId = $('#bankChargesWalletId').val();
			options.TDSreceivableWalletId = $('#TDSreceivableWalletId').val();
...
// Add program asset (opens only program UI)
function addAsset()
{
console.log("Inside add Asset");
    $.when($.get('createAsset.html')).done(function (_page)
    {
        $('#admin-forms').empty();
        $('#admin-forms').append(_page);
        updatePage('asset');
        ...
        _cancel.on('click', function (){$('#admin-forms').empty();});
        _submit.on('click', function(){
            $('#messages').append('<br/>starting add asset request.');
            let options = {};
       ...
       // options.type = $('#member_type').find(':selected').text();
			...
			options.programStart_Date = $('#programStart_Date').val();
			options.programEnd_Date = $('#programEnd_Date').val();
			...
			options.repaymentWalletId = $('#repaymentWalletId').val();
			
			options.discountPeriod = $('#discountPeriod').val();
			options.discountPercent = $('#discountPercent').val();
