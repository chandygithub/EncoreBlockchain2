...
func newTxnInfo(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	payString := strconv.Itoa(int(walletResponse.Payload[0]))
	openBal, err := strconv.ParseInt(payString, 10, 64)
	if err != nil {
		return shim.Error("Error in converting the balance in bank main wallet")
	}
	cAmt := int64(0)
	dAmt := amt
	...
	argsList := []string{"1", args[0], args[2], args[3], args[4], walletID, payString, args[1], args[5], "0", args[5], txnBalString, args[8]}
	...
	openBal, err = strconv.ParseInt(payString, 10, 64)
	if err != nil {
		return shim.Error("Error in converting the balance business main wallet")
	}
	cAmt = amt
	dAmt = 0
	txnBal = openBal - dAmt + cAmt
	...
	// STEP-4 generate txn_balance_object and write it to the Txn_Bal_Ledger
	argsList = []string{"2", args[0], args[2], args[3], args[4], walletID, payString, args[1], args[5], args[5], "0", txnBalString, args[8]}
	argsListStr = strings.Join(argsList, ",")
	chaincodeArgs = util.ToChaincodeArgs("putTxnInfo", argsListStr)
	fmt.Println("calling the other chaincode")
	response = stub.InvokeChaincode("txnbalcc", chaincodeArgs, "myc")
	...
	// getting Balance from walletID
	// walletFcn := "getWallet"
	fmt.Println("IN step 2 of Business Loan wallet")
	walletArgs = util.ToChaincodeArgs("getWallet", walletID)
	walletResponse = stub.InvokeChaincode("walletcc", walletArgs, "myc")
	if walletResponse.Status != shim.OK {
		return shim.Error(walletResponse.Message)
	...
	openBal, err = strconv.ParseInt(payString, 10, 64)
	if err != nil {
		return shim.Error("Error in converting the balance business loan wallet")
	}
	cAmt = amt
	dAmt = 0
	txnBal = openBal - dAmt + cAmt
	txnBalString = strconv.FormatInt(txnBal, 10)
	// STEP-3
	fmt.Println("IN step 3 of Business Loan wallet")
	// update wallet of ID walletID here, and write it to the wallet_ledger
	// walletFcn := "updateWallet"
	walletArgs = util.ToChaincodeArgs("updateWallet", walletID, string(txnBal))
	walletResponse = stub.InvokeChaincode("walletcc", walletArgs, "myc")
	...
	// STEP-4 generate txn_balance_object and write it to the Txn_Bal_Ledger
	fmt.Println("IN step 4 of Business Loan wallet")
	argsList = []string{"3", args[0], args[2], args[3], args[4], walletID, payString, args[1], args[5], args[5], "0", txnBalString, args[8]}
	argsListStr = strings.Join(argsList, ",")
	chaincodeArgs = util.ToChaincodeArgs("putTxnInfo", argsListStr)
	fmt.Println("calling the other chaincode")
	response = stub.InvokeChaincode("txnbalcc", chaincodeArgs, "myc")
	...
	openBal, err = strconv.ParseInt(payString, 10, 64)
	if err != nil {
		return shim.Error("Error in converting the balance bank asset wallet")
	}
	cAmt = amt
	dAmt = 0
	txnBal = openBal - dAmt + cAmt
	...
	// STEP-4 generate txn_balance_object and write it to the Txn_Bal_Ledger
	argsList = []string{"4", args[0], args[2], args[3], args[4], walletID, payString, args[1], args[5], args[5], "0", txnBalString, args[8]}
	argsListStr = strings.Join(argsList, ",")
	chaincodeArgs = util.ToChaincodeArgs("putTxnInfo", argsListStr)
	fmt.Println("calling the other chaincode")
	response = stub.InvokeChaincode("txnbalcc", chaincodeArgs, "myc")
...
func getTxnInfo(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	fmt.Printf("Transactions ID %s : %s\n", args[0], tString)
	return shim.Success(nil)
